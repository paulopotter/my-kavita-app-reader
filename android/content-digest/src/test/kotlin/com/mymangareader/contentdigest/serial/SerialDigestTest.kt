...
private fun fakeChapter(
    ...
    specialLabel = if (isSpecial) "Extra" else decimalNumber.toString(),
...
private class FakePlugin(
    var serialResult: Result<PluginSerial> =
        Result.success(
            PluginSerial(
                id = "s1",
                name = "Series 1",
                coverUrl = "http://cover/s1",
                pagesRead = 0,
                totalPages = 0,
                libraryId = "1",
                libraryName = "Library",
    ...
    var metadataResult: Result<PluginSeriesMetadata> =
        Result.success(
...
private class FakeExternalMetadataPlugin(
    var matchResult: Result<ExternalMetadataMatch?>,
    ...
    override suspend fun fetchMatches(series: List<ExternalMetadataSeriesRef>): List<ExternalMetadataMatch?> = listOf(matchResult.getOrThrow())
...
private fun fakeExternalMetadataRegistration(plugin: FakeExternalMetadataPlugin): ExternalMetadataPluginRegistration =
    object : ExternalMetadataPluginRegistration {
        ...
        override val factory = {
                _: RequestTool,
                _: com.mymangareader.cache.Cache,
                _: String,
                _: String,
            ->
...
class SerialDigestTest {
    ...
    @Test
    fun `success carries every series field`() =
        runTest {
            activateGroup()
    ...
    @Test
    fun `full defaults to false — each chapter's pages list comes back empty`() =
        runTest {
            activateGroup()
    ...
    @Test
    fun `full=true propagates down to every chapter's pages list`() =
        runTest {
            activateGroup()
    ...
    @Test
    fun `serial get failure makes the whole result a Failure`() =
        runTest {
            activateGroup()
    ...
    @Test
    fun `getMetadata failure is tolerated — Success with metadata null`() =
        runTest {
            activateGroup()
    ...
    @Test
    fun `chapters list failure is tolerated — Success with chapters null`() =
        runTest {
            activateGroup()
    ...
    @Test
    fun `library is null when libraryId is missing`() =
        runTest {
            activateGroup()
            plugin.serialResult =
                Result.success(
    ...
    @Test
    fun `chapters are sorted by decimalNumber, with a special chapter landing in the right place`() =
        runTest {
            activateGroup()
    ...
    @Test
    fun `number reflects 1-indexed position in the sorted list, not decimalNumber`() =
        runTest {
            activateGroup()
            plugin.chaptersListResult =
                Result.success(
    ...
    @Test
    fun `prevChapter and nextChapter point to the correct sorted neighbors`() =
        runTest {
            activateGroup()
            plugin.chaptersListResult =
                Result.success(
    ...
    @Test
    fun `chapters status is SUCCESS when every chapter succeeds`() =
        runTest {
            activateGroup()
    ...
    @Test
    fun `readCount is null for a genuinely empty chapters list`() =
        runTest {
            activateGroup()
    ...
    @Test
    fun `readCount counts only chapters whose readStatus is READ`() =
        runTest {
            activateGroup()
            plugin.chaptersListResult =
                Result.success(
    ...
    @Test
    fun `resumePoint picks the first IN_PROGRESS chapter over UNREAD ones`() =
        runTest {
            activateGroup()
            plugin.chaptersListResult =
                Result.success(
    ...
    @Test
    fun `resumePoint falls back to the first UNREAD chapter when none are IN_PROGRESS`() =
        runTest {
            activateGroup()
            plugin.chaptersListResult =
                Result.success(
    ...
    @Test
    fun `resumePoint's recordedAtEpochMs is null when the chapter has no progress data at all`() =
        runTest {
            activateGroup()
            val unreadNoProgress =
    ...
    @Test
    fun `resumePoint is null when every chapter is READ`() =
        runTest {
            activateGroup()
            plugin.chaptersListResult =
                Result.success(
    ...
    @Test
    fun `chapters status is PARTIAL when one chapter fails and others succeed`() =
        runTest {
            activateGroup()
            plugin.chaptersListResult =
                Result.success(
    ...
    @Test
    fun `chapters status is ERROR when every chapter fails`() =
        runTest {
            activateGroup()
            plugin.chaptersListResult =
                Result.success(
    ...
    @Test
    fun `a failed chapter's neighbors still get built for the surrounding successful chapters`() =
        runTest {
            activateGroup()
            plugin.chaptersListResult =
                Result.success(
    ...
    @Test
    fun `includeExternalMetadata false (default) never calls buildExternalMetadataDigest — metadata external stays null`() =
        runTest {
            activateGroup()
    ...
    @Test
    fun `includeExternalMetadata true resolves via syncByServerId using this series' own serverInfo groupId`() =
        runTest {
            activateGroup()
            val kavitaGroupId = server.getActiveGroupId()!!
            val externalPlugin =
                FakeExternalMetadataPlugin(
    ...
    @Test
    fun `externalMetadataGroupId override uses syncByGroup instead of resolving by server`() =
        runTest {
            activateGroup()
            val externalPlugin =
                FakeExternalMetadataPlugin(
            ...
            val digest =
                buildSerialDigest(
                    ...
                    SerialDigestOptions(
                        ...
                        externalMetadataGroupId = explicitGroup.id,
                    ),
                ) as SerialDigest.Success
    ...
    @Test
    fun `includeExternalMetadata true with no group configured yields a Failure with not_configured code`() =
        runTest {
            activateGroup()
            val extGroupDao = FakeExternalMetadataGroupDao()
            val extUrlDao = FakeExternalMetadataUrlDao()
            val emptyExternalMetadataServer =
                ExternalMetadataServer(
                    extGroupDao,
                    extUrlDao,
                    emptyMap(),
    ...
    @Test
    fun `external metadata sync failure becomes a Failure, not a null, and doesn't fail the whole digest`() =
        runTest {
            activateGroup()
            val kavitaGroupId = server.getActiveGroupId()!!
    ...
    @Test
    fun `full and includeExternalMetadata variants are cached separately`() =
        runTest {
            activateGroup()
...
class SerialsDigestTest {
    ...
    @Test
    fun `every listed series comes back as a minimal SerialDigest Success`() =
        runTest {
            activateGroup()
    ...
    @Test
    fun `each series is written into its own per-series cache under the single-series key`() =
        runTest {
            activateGroup()
    ...
    @Test
    fun `lastUpdatedEpochMs is the newest cachedAtEpochMs across the per-series caches`() =
        runTest {
            activateGroup()
            plugin.serialsListResult = Result.success(listOf(fakePluginSerial("s1"), fakePluginSerial("s2")))
    ...
    @Test
    fun `empty list yields Success with no serials and a null lastUpdatedEpochMs`() =
        runTest {
            activateGroup()
    ...
    @Test
    fun `a serials list failure makes the whole result a Failure`() =
        runTest {
            activateGroup()
    ...
    @Test
    fun `buildSerialDigest(id) after a list-only seed re-fetches instead of serving the chapterless entry`() =
        runTest {
            activateGroup()
    ...
    @Test
    fun `a list refresh shallow-merges into the per-series cache — chapters a prior buildSerialDigest wrote survive on disk`() =
        runTest {
            activateGroup()
