...
private class FakePlugin(
    var chapterResult: Result<PluginChapter> =
        Result.success(
            PluginChapter(
                id = "c1",
                title = "Chapter 1",
...
class ChapterDigestTest {
    ...
    @Test
    fun `success carries every chapter field`() =
        runTest {
            activateGroup()
    ...
    @Test
    fun `chapter get failure makes the whole result a Failure`() =
        runTest {
            activateGroup()
    ...
    @Test
    fun `getProgress failure is tolerated — Success with null stoppedAtPageIndex`() =
        runTest {
            activateGroup()
    ...
    @Test
    fun `resumePoint carries stoppedAtPageIndex and recordedAtEpochMs when both are available`() =
        runTest {
            activateGroup()
    ...
    @Test
    fun `resumePoint is null when neither stoppedAtPageIndex nor recordedAtEpochMs are available`() =
        runTest {
            activateGroup()
    ...
    @Test
    fun `number is decimalNumber truncated when it's a whole number`() =
        runTest {
            activateGroup()
    ...
    @Test
    fun `number is null when decimalNumber is fractional`() =
        runTest {
            activateGroup()
    ...
    @Test
    fun `number is null when decimalNumber is null`() =
        runTest {
            activateGroup()
    ...
    @Test
    fun `specialLabel is populated only when isSpecial is true`() =
        runTest {
            activateGroup()
    ...
    @Test
    fun `specialLabel is null when isSpecial is false, even if specialLabel is present`() =
        runTest {
            activateGroup()
    ...
    @Test
    fun `specialLabel is null when isSpecial is null`() =
        runTest {
            activateGroup()
    ...
    @Test
    fun `readStatus is UNREAD when readCount is zero`() =
        runTest {
            activateGroup()
    ...
    @Test
    fun `readStatus is READ when readCount reaches count`() =
        runTest {
            activateGroup()
    ...
    @Test
    fun `readStatus is IN_PROGRESS when readCount is between zero and count`() =
        runTest {
            activateGroup()
    ...
    @Test
    fun `readStatus falls back to UNREAD when count or readCount is null`() =
        runTest {
            activateGroup()
    ...
    @Test
    fun `pages list is built by calling Page per index, in order`() =
        runTest {
            activateGroup()
    ...
    @Test
    fun `pages status is SUCCESS when every page succeeds`() =
        runTest {
            activateGroup()
    ...
    @Test
    fun `pages status is ERROR when every page's getUrl fails`() =
        runTest {
            activateGroup()
            plugin.chapterResult = Result.success(baseChapter(pageCount = 2))
    ...
    @Test
    fun `pages status is PARTIAL when some pages fail getUrl and others succeed`() =
        runTest {
            activateGroup()
            plugin.chapterResult = Result.success(baseChapter(pageCount = 3))
            plugin.urlForPage =
                { index -> if (index == 1) Result.failure(RuntimeException("bad")) else Result.success("http://fake/page/$index") }
    ...
    @Test
    fun `a getDimensions failure on one page is tolerated and doesn't affect pages status`() =
        runTest {
            activateGroup()
            plugin.chapterResult = Result.success(baseChapter(pageCount = 3))
            plugin.dimensionsForPage =
                { index -> if (index == 1) Result.failure(RuntimeException("bad")) else Result.success(PluginPageDimension(800, 1200)) }
    ...
    @Test
    fun `full defaults to false — pages list is empty, status and total are null`() =
        runTest {
            activateGroup()
            plugin.chapterResult = Result.success(baseChapter(pageCount = 3))
    ...
    @Test
    fun `totalWidthPx and totalHeightPx are summed when every page has usable dimensions`() =
        runTest {
            activateGroup()
            plugin.chapterResult = Result.success(baseChapter(pageCount = 2))
    ...
    @Test
    fun `totalWidthPx and totalHeightPx are null when a page has zero dimensions`() =
        runTest {
            activateGroup()
            plugin.chapterResult = Result.success(baseChapter(pageCount = 2))
            plugin.dimensionsForPage =
                { index -> if (index == 0) Result.success(PluginPageDimension(0, 0)) else Result.success(PluginPageDimension(100, 200)) }
    ...
    @Test
    fun `totalWidthPx and totalHeightPx are null when pages list is empty`() =
        runTest {
            activateGroup()
    ...
    @Test
    fun `prevChapter and nextChapter are carried through unchanged (Failure neighbor)`() =
        runTest {
            activateGroup()
            val neighbor =
                ChapterNeighborDigest.Failure(
                    com.mymangareader.contentdigest.error
                        .ErrorDigest("X", "boom"),
                )
    ...
    @Test
    fun `prevChapter and nextChapter are carried through unchanged (Success neighbor)`() =
        runTest {
            activateGroup()
            ...
            val digest =
                buildChapterDigest(
                    ...
                    "c1",
                    cache,
                    prevChapter = neighbor,
                    nextChapter = neighbor,
                ) as ChapterDigest.Success
    ...
    @Test
    fun `a complete knownChapter skips chapter get entirely`() =
        runTest {
            activateGroup()
    ...
    @Test
    fun `an incomplete knownChapter (one field missing) falls back to chapter get entirely`() =
        runTest {
            activateGroup()
    ...
    @Test
    fun `no knownChapter always calls chapter get, same as before`() =
        runTest {
            activateGroup()
    ...
    @Test
    fun `server and resolvedAtEpochMs come from getCoverImage when chapter get was skipped`() =
        runTest {
            activateGroup()
            val known = baseChapter()
    ...
    @Test
    fun `a write with no neighbors preserves neighbors already attached by a previous write`() =
        runTest {
            activateGroup()
            val neighbor =
                ChapterNeighborDigest.Failure(
    ...
    @Test
    fun `a write with a neighbor overwrites whatever was cached before`() =
        runTest {
            activateGroup()
            val oldNeighbor =
                ChapterNeighborDigest.Failure(
    ...
    @Test
    fun `full=true and full=false are cached separately, never mixed up`() =
        runTest {
            activateGroup()
