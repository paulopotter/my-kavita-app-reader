 ...
 * status, parsing) propagate as real exceptions up through the caller chain until whoever
 * actually needs to handle them (the future Server) decides whether to catch or let it keep
 * rising. No `try`/`catch` boilerplate is expected inside this class or its callers here.
 */
class KavitaAuth(
    private val baseUrl: String,
    private val requestTool: RequestTool,
) {
    suspend fun authenticate(apiKey: String): KavitaUserDto {
        val url = "$baseUrl$AUTHENTICATE_PATH?apiKey=$apiKey&pluginName=$PLUGIN_NAME"
        val http = requestTool.request(url = url, method = "POST").getOrThrow()

        return when {
            http.status == 200 -> authJson.decodeFromString<KavitaUserDto>(http.body)
            http.status == 401 -> throw KavitaAuthException("Invalid API key (401)")
            else -> throw KavitaAuthException("Authentication failed: HTTP ${http.status}")
        }
    }
