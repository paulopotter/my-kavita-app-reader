...
private fun validateCredentials(
    ...
    val parsed =
        runCatching { authFormat.parseToJsonElement(credentialsJson).jsonObject }
...
private fun requireValidGroupFields(
    name: String?,
...
private fun PluginSerial.toSerialData(
...
class Server
    ...
    constructor(
            ...
            object : Groups {
                ...
                override suspend fun update(
                    ...
                    if (credentialsJson != null) {
                        val registration =
                            pluginRegistrations[existing.providerId]
        ...
        private inner class SerialHandle(
            ...
            override suspend fun getMetadata(): ServerResponse<PluginSeriesMetadata> = withUrlRetryEnveloped { it.serial(serialId).getMetadata() }
        ...
        private inner class ChapterHandle(
            private val serialId: String,
            ...
            override suspend fun getProgress(): ServerResponse<PluginProgress?> = withUrlRetryEnveloped { it.serial(serialId).chapter(chapterId).getProgress() }
        ...
        private suspend fun <T> withUrlRetryEnveloped(action: suspend (ServerPlugin) -> T): ServerResponse<T> {
            ...
            val groupId = activeGroupId ?: throw ServerException("No active server group set — call setActiveGroup(id) first")
            val serverInfo =
                activeMutex.withLock { lastActiveInfoByGroupId[groupId] }
                    ?: throw ServerException(
                        "No resolution recorded for group $groupId after a successful content call — this should be unreachable",
                    )
        ...
        private inner class GroupHandle(
            ...
            override suspend fun updateUrl(
                ...
                val existing =
                    serverUrlDao.getById(urlId)?.takeIf { it.groupId == groupId }
            ...
            override suspend fun getActive(): ServerUrlInfo? =
                activeMutex.withLock {
...
private fun ServerPluginRegistration.toInfo() =
    ProviderInfo(
        id = id,
...
private fun ServerGroupEntity.toInfo() =
    ServerGroupInfo(
        id = id,
        name = name,
...
private fun ServerUrlEntity.toInfo() =
    ServerUrlInfo(
        id = id,
        groupId = groupId,
        url = url,
...
private fun ServerUrlEntity.toUrlCandidate(healthCheckPath: String) =
    UrlCandidate(
        id = id,
        url = url,
        timeoutMs = timeoutMs,
