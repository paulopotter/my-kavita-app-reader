...
class KavitaSeriesTest {
    ...
    @Test
    fun `listSeries returns full raw dto list`() =
        runTest {
            server.enqueue(
    ...
    @Test
    fun `getSeries throws on non-200`() =
        runTest {
    ...
    @Test
    fun `getSeriesMetadata returns genres and tags with ids`() =
        runTest {
            server.enqueue(
                MockResponse().setResponseCode(200).setBody(
    ...
    @Test
    fun `getSeriesMetadata throws on non-200`() =
        runTest {
            server.enqueue(MockResponse().setResponseCode(500))
