...
internal class MemoryKotlinHandle : MemoryKotlin {
    ...
    override suspend fun get(key: String, variant: String): CacheEntry? = mutex.withLock {
    ...
    override suspend fun put(key: String, value: String, domain: String, variant: String, ttlMs: Long): CacheDescriptor {
        ...
        mutex.withLock {
            entries[MapKey(key, variant)] = Entry(
                value = value,
                domain = domain,
                cachedAtEpochMs = now,
                ttlMs = ttlMs,
    ...
    override suspend fun invalidate(key: String, variant: String): Unit = mutex.withLock { entries.remove(MapKey(key, variant)); Unit }
    ...
    override suspend fun invalidateVariant(domain: String, variant: String) = mutex.withLock {
    ...
    override suspend fun purgeOlderThan(cutoffEpochMs: Long) = mutex.withLock {
