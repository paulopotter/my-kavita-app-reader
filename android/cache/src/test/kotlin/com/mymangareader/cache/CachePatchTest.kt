...
class CachePatchTest {
    ...
    @Test
    fun `patch on a missing key behaves like put`() = runTest {
    ...
    @Test
    fun `patch keeps fields the new value does not mention and overwrites the ones it does`() = runTest {
        val cache = PersistentHandle(FakeCacheDao())
    ...
    @Test
    fun `shallow (default) replaces nested objects wholesale`() = runTest {
        val cache = PersistentHandle(FakeCacheDao())
    ...
    @Test
    fun `deep = true merges nested objects recursively, keeping sibling keys`() = runTest {
        val cache = PersistentHandle(FakeCacheDao())
    ...
    @Test
    fun `deep = true still replaces a value wholesale when the types differ (object vs scalar)`() = runTest {
        val cache = PersistentHandle(FakeCacheDao())
    ...
    @Test
    fun `deep = true never element-merges arrays`() = runTest {
        val cache = PersistentHandle(FakeCacheDao())
    ...
    @Test
    fun `patch degrades to put when the existing value is not a JSON object`() = runTest {
        val cache = PersistentHandle(FakeCacheDao())
    ...
    @Test
    fun `patch degrades to put when the new value is not a JSON object`() = runTest {
        val cache = PersistentHandle(FakeCacheDao())
    ...
    @Test
    fun `patch refreshes cachedAt and returns a descriptor`() = runTest {
        val cache = PersistentHandle(FakeCacheDao())
    ...
    @Test
    fun `variant scopes the entry patch touches`() = runTest {
        val cache = PersistentHandle(FakeCacheDao())
    ...
    @Test
    fun `patchAll merges each item and returns descriptors in item order`() = runTest {
        val cache = PersistentHandle(FakeCacheDao())
        cache.put("s1", """{"name":"A","chapters":{"total":3}}""", domain = "serial", variant = "v")
        cache.put("s2", """{"name":"B"}""", domain = "serial", variant = "v")
        // s3 is new

        val descriptors = cache.patchAll(
            listOf(
    ...
    @Test
    fun `patchAll with a domain-scoped readFilter still merges the right existing rows`() = runTest {
        val cache = PersistentHandle(FakeCacheDao())
    ...
    @Test
    fun `patchAll on an empty list is a no-op`() = runTest {
        val cache = PersistentHandle(FakeCacheDao())
        assertEquals(emptyList(), cache.patchAll(emptyList(), domain = "serial"))
    }

    @Test
    fun `patch and a one-item patchAll produce the same merged result`() = runTest {
