...
class NetworkCacheTest {
    @Test
    fun `run executa o block e retorna seu resultado`() =
        runTest {
    ...
    @Test
    fun `run dentro do ttl reusa o resultado anterior, sem executar block de novo`() =
        runTest {
            val network = NetworkHandle()
            val calls = AtomicInteger(0)

            network.run("k", ttlMs = 60_000L) {
    ...
    @Test
    fun `run apos o ttl expirar executa o block novamente`() =
        runTest {
            val network = NetworkHandle()
            val calls = AtomicInteger(0)

            network.run("k", ttlMs = -1L) {
                calls.incrementAndGet()
                "first"
            }
            val second =
    ...
    @Test
    fun `keys diferentes nao compartilham cache nem lock entre si`() =
        runTest {
            val network = NetworkHandle()
    ...
    @Test
    fun `chamadas concorrentes com a mesma key executam block uma unica vez (dedup in-flight)`() =
        runTest {
            val network = NetworkHandle()
            val calls = AtomicInteger(0)
    ...
    @Test
    fun `chamadas concorrentes com keys diferentes nao se bloqueiam mutuamente`() =
        runTest {
            val network = NetworkHandle()
            val calls = AtomicInteger(0)
    ...
    @Test
    fun `invalidate forca o proximo run a executar block de novo, mesmo dentro do ttl`() =
        runTest {
            val network = NetworkHandle()
            val calls = AtomicInteger(0)
            network.run("k", ttlMs = 60_000L) {
    ...
    @Test
    fun `invalidate de uma key nao afeta outras`() =
        runTest {
            val network = NetworkHandle()
            ...
            val calls = AtomicInteger(0)
            network.run("a", ttlMs = 60_000L) {
                calls.incrementAndGet()
                "value-a-2"
            }
            val b =
                network.run("b", ttlMs = 60_000L) {
                    calls.incrementAndGet()
                    "should-not-run"
                }
    ...
    fun `purgeExpired remove somente entradas expiradas`() =
        runTest {
            ...
            val calls = AtomicInteger(0)
            val expired =
                network.run("expired", ttlMs = 60_000L) {
                    calls.incrementAndGet()
                    "re-fetched"
                }
            val fresh =
                network.run("fresh", ttlMs = 60_000L) {
                    calls.incrementAndGet()
                    "should-not-run"
                }
    ...
    @Test
    fun `purgeOlderThan nao remove entrada gravada antes do cutoff mas lida depois dele`() =
        runTest {
            val network = NetworkHandle()
            ...
            val calls = AtomicInteger(0)
            val result =
                network.run("stale-but-read", ttlMs = 60_000L) {
                    calls.incrementAndGet()
                    "re-fetched"
                }
    ...
    @Test
    fun `purgeOlderThan remove entrada nunca lida de novo antes do cutoff`() =
        runTest {
            val network = NetworkHandle()
            ...
            val calls = AtomicInteger(0)
            val result =
                network.run("never-read-again", ttlMs = 60_000L) {
                    calls.incrementAndGet()
                    "re-fetched"
                }
