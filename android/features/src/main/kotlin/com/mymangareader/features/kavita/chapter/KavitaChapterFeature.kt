...
class KavitaChapterFeature
    ...
    constructor(
                    ...
                    volumes.flatMap { volume ->
                        volume.chapters.map { ch ->
                            ...
                            ChapterCacheEntity(
                                ...
                                readStatus =
                                    readStatusById[chId]
                                        ?: if (ch.pagesRead >= ch.pages &&
                                            ch.pages > 0
                                        ) {
                                            "READ"
                                        } else if (ch.pagesRead > 0) {
                                            "IN_PROGRESS"
                                        } else {
                                            "UNREAD"
                                        },
        ...
        override suspend fun getPageUrls(
            chapterId: String,
        ...
        override suspend fun invalidatePageCache(chapterId: String): Result<Unit> = runCatching { pageCacheDao.deleteByChapterId(chapterId) }

        override suspend fun getPageCacheUrls(chapterId: String): Result<List<Pair<Int, String>>> = runCatching { pageCacheDao.getByChapterId(chapterId).map { it.pageIndex to it.url } }
        ...
        override suspend fun saveLocalProgress(
            chapterId: String,
            seriesId: String,
            page: Int,
        ...
        ): Result<Unit> = markChapters(seriesId, chapterIds, MARK_MULTIPLE_READ_PATH, readStatus = "READ")

        suspend fun markChaptersUnread(
            seriesId: String,
            chapterIds: List<String>,
        ): Result<Unit> = markChapters(seriesId, chapterIds, MARK_MULTIPLE_UNREAD_PATH, readStatus = "UNREAD")
