...
private class FakeUrlSource(
    ...
    override suspend fun getActiveUrl(): Result<String> = if (urls.isEmpty()) Result.success(urls.lastOrNull() ?: "") else Result.success(urls.removeAt(0))
...
class ActiveUrlWatcherTest {
    @Test
    fun `emite apenas quando a url muda entre polls`() =
        runTest {
    ...
    @Test
    fun `start chamado duas vezes nao inicia dois jobs`() =
        runTest {
    ...
    @Test
    fun `valor inicial e nulo antes do primeiro poll`() =
        runTest {
