...
class KavitaSeriesFeatureTest {
    ...
    @Test
    fun `getSeriesDetail retorna detalhe da serie em caso de sucesso`() =
        runTest {
            server.enqueue(
    ...
    @Test
    fun `getSeriesDetail retorna failure em erro HTTP`() =
        runTest {
    ...
    @Test
    fun `getSeriesDetail retorna failure quando nao autenticado`() =
        runTest {
    ...
    @Test
    fun `getSeriesMetadata retorna summary, generos e tags em caso de sucesso`() =
        runTest {
            server.enqueue(
                MockResponse().setResponseCode(200).setBody(
    ...
    @Test
    fun `getSeriesMetadata usa seriesId como query param`() =
        runTest {
    ...
    @Test
    fun `getSeriesMetadata retorna summary nulo e listas vazias quando campos ausentes`() =
        runTest {
            server.enqueue(MockResponse().setResponseCode(200).setBody("""{"seriesId":42}"""))
    ...
    @Test
    fun `getSeriesMetadata retorna failure em erro HTTP`() =
        runTest {
    ...
    @Test
    fun `getCachedSeriesDetail retorna nulo quando nada foi cacheado ainda`() =
        runTest {
            assertEquals(null, feature.getCachedSeriesDetail("42"))
        }

    @Test
    fun `getSeriesDetail bem sucedido persiste no cache local`() =
        runTest {
    ...
    @Test
    fun `getSeriesDetail com falha nao sobrescreve cache existente`() =
        runTest {
            server.enqueue(MockResponse().setResponseCode(200).setBody("""{"id":42,"name":"One Piece"}"""))
            feature.getSeriesDetail("42")
            server.enqueue(MockResponse().setResponseCode(500))
    ...
    @Test
    fun `getSeriesMetadata bem sucedido persiste generos e tags no cache local`() =
        runTest {
            server.enqueue(
                MockResponse().setResponseCode(200).setBody(
    ...
    @Test
    fun `cache de detail e metadata sao mesclados na mesma entity sem se sobrescreverem`() =
        runTest {
            server.enqueue(MockResponse().setResponseCode(200).setBody("""{"id":42,"name":"One Piece"}"""))
            server.enqueue(
                MockResponse().setResponseCode(200).setBody(
