...
class ActiveUrlSelectorTest {
    ...
    @Test
    fun `returns the single healthy candidate`() =
        runTest {
    ...
    @Test
    fun `returns failure when no candidate responds`() =
        runTest {
            server1.shutdown()
            server2.shutdown()

            val result =
                selector.getActiveUrl(
                    listOf(
    ...
    @Test
    fun `returns failure on empty candidate list`() =
        runTest {
    ...
    @Test
    fun `cached result is returned without new health check`() =
        runTest {
            server1.enqueue(MockResponse().setResponseCode(200))
    ...
    @Test
    fun `invalidateAndReselect re-probes after cache is cleared`() =
        runTest {
            server1.enqueue(MockResponse().setResponseCode(200))
            ...
            val second =
                selector.invalidateAndReselect(
                    listOf(
    ...
    @Test
    fun `getLastKnownUrl returns cached url after successful probe`() =
        runTest {
            server1.enqueue(MockResponse().setResponseCode(200))
    ...
    // hanging.
    @Test
    fun `a candidate that never responds is treated as unhealthy, not left hanging`() =
        runTest {
    ...
    @Test
    fun `a slower never-responding candidate does not block a healthy one from winning`() =
        runTest {
            server1.enqueue(MockResponse().setSocketPolicy(SocketPolicy.NO_RESPONSE))
            server2.enqueue(MockResponse().setResponseCode(200))
    ...
    @Test
    fun `probe reports ok with the response status on a healthy URL`() =
        runTest {
            server1.enqueue(MockResponse().setResponseCode(200))
    ...
    @Test
    fun `probe reports not-ok with the status on a non-2xx response`() =
        runTest {
    ...
    @Test
    fun `probe reports not-ok with a null status when the URL is unreachable`() =
        runTest {
            server1.shutdown()
    ...
    @Test
    fun `probe never populates the selection cache`() =
        runTest {
            server1.enqueue(MockResponse().setResponseCode(200))
