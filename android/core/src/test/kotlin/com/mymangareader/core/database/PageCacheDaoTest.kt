...
class PageCacheDaoTest {
    ...
    @Test
    fun `getByChapterId retorna vazio quando nao ha cache`() =
        runTest {
            assertTrue(dao.getByChapterId("c1").isEmpty())
        }

    @Test
    fun `replaceForChapter insere paginas ordenadas por pageIndex`() =
        runTest {
    ...
    @Test
    fun `replaceForChapter substitui em vez de acumular`() =
        runTest {
    ...
    @Test
    fun `countByChapterId reflete numero de paginas cacheadas`() =
        runTest {
            dao.replaceForChapter(
                "c1",
                listOf(
