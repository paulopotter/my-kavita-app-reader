...
class CacheDaoTest {
    ...
    @Test
    fun `getByKey retorna null quando a chave nao existe`() =
        runTest {
            assertNull(dao.getByKey("missing", ""))
        }

    @Test
    fun `upsert seguido de getByKey retorna a entrada gravada`() =
        runTest {
    ...
    @Test
    fun `upsert com a mesma chave e variant substitui a entrada anterior`() =
        runTest {
    ...
    @Test
    fun `deleteByKey remove apenas a entrada indicada`() =
        runTest {
    ...
    @Test
    fun `deleteByDomain remove todas as entradas daquele dominio, preservando outros`() =
        runTest {
    ...
    @Test
    fun `deleteByVariant remove todas as entradas daquele dominio e variant, preservando outras variants e dominios`() =
        runTest {
    ...
    @Test
    fun `touchLastAccessed atualiza somente a entrada indicada`() =
        runTest {
    ...
    @Test
    fun `getOlderThan retorna apenas entradas com cachedAt e lastAccessedAt ambos antes do cutoff`() =
        runTest {
    ...
    @Test
    fun `getAllExpired retorna apenas entradas com expiresAtEpochMs menor ou igual a agora`() =
        runTest {
    ...
    @Test
    fun `deleteExpired remove multiplas entradas de uma vez, respeitando key e variant`() =
        runTest {
            dao.upsert(entry("a"))
            dao.upsert(entry("b"))
    ...
    @Test
    fun `queryFiltered with hasKeys 0 and null domain,variant returns every row`() =
        runTest {
    ...
    @Test
    fun `queryFiltered ANDs keys, domain and variant`() =
        runTest {
    ...
    @Test
    fun `upsertAllLenient writes every row and returns the count`() =
        runTest {
            val written =
                dao.upsertAllLenient(
    ...
    @Test
    fun `upsertAllLenient replaces an existing row (same key,variant)`() =
        runTest {
