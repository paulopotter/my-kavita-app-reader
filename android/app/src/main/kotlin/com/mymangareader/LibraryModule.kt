...
class LibraryModule @Inject constructor(
    ...
    fun listSeries(forceRefresh: Boolean, promise: Promise) {
        scope.launch {
            ...
            if (!forceRefresh && cached.isNotEmpty() && (now - lastFetchMs) < cacheTtlMs) {
                promise.resolve(buildSeriesArray(cached))
                return@launch
            }
            kavitaSeriesFeature.listSeries()
                .onSuccess { series ->
                    ...
                    promise.resolve(buildSeriesArray(series))
                }
    ...
    private fun buildSeriesArray(series: List<SeriesSummary>) = Arguments.createArray().also { array ->
        series.forEach { s ->
