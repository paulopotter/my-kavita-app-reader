...
fun ServerActiveInfo.toWritableMap(): WritableMap =
...
fun ImageDescriptor.toWritableMap(): WritableMap =
    Arguments.createMap().apply {
        putString("url", url)
...
fun ErrorDigest.toWritableMap(): WritableMap =
    Arguments.createMap().apply {
...
fun PageDigest.toWritableMap(): WritableMap =
    when (this) {
        is PageDigest.Failure ->
            Arguments.createMap().apply {
                putBoolean("isSuccess", false)
                putMap("error", error.toWritableMap())
            }
        is PageDigest.Success ->
            Arguments.createMap().apply {
...
private fun WritableMap.putChapterFields(fields: ChapterFields) =
    apply {
...
private fun ChapterFields.Pages.toWritableMap(): WritableMap =
    Arguments.createMap().apply {
...
private fun ChapterFields.ResumePoint.toWritableMap(): WritableMap =
    Arguments.createMap().apply {
        stoppedAtPageIndex?.let { putInt("stoppedAtPageIndex", it) }
        recordedAtEpochMs?.let { putDouble("recordedAtEpochMs", it.toDouble()) }
    }

fun ChapterDigest.toWritableMap(): WritableMap =
    when (this) {
        is ChapterDigest.Failure ->
            Arguments.createMap().apply {
                putBoolean("isSuccess", false)
                putMap("error", error.toWritableMap())
            }
...
fun ChapterNeighborDigest.toWritableMap(): WritableMap =
    when (this) {
        is ChapterNeighborDigest.Failure ->
            Arguments.createMap().apply {
                putBoolean("isSuccess", false)
                putMap("error", error.toWritableMap())
            }
        is ChapterNeighborDigest.Success ->
            Arguments.createMap().apply {
                putBoolean("isSuccess", true)
                putChapterFields(this@toWritableMap)
            }
...
fun PluginGenreOrTag.toWritableMap(): WritableMap =
    Arguments.createMap().apply {
        putString("id", id)
        putString("name", name)
    }

fun PluginAgeRating.toWritableMap(): WritableMap =
    Arguments.createMap().apply {
        rating?.let { putString("rating", it) }
        putString("system", system)
    }

private fun SerialFields.Library.toWritableMap(): WritableMap =
    Arguments.createMap().apply {
        putString("id", id)
        name?.let { putString("name", it) }
    }

private fun SerialFields.LastUpdatesUTC.toWritableMap(): WritableMap =
    Arguments.createMap().apply {
...
private fun SerialFields.OtherNames.toWritableMap(): WritableMap =
    Arguments.createMap().apply {
        original?.let { putString("original", it) }
        localized?.let { putString("localized", it) }
    }

private fun SerialFields.OtherIds.toWritableMap(): WritableMap =
    Arguments.createMap().apply {
        aniListId?.let { putInt("aniListId", it) }
        malId?.let { putDouble("malId", it.toDouble()) }
    }

private fun SerialFields.Colors.toWritableMap(): WritableMap =
    Arguments.createMap().apply {
        primary?.let { putString("primary", it) }
        secondary?.let { putString("secondary", it) }
    }

private fun SerialFields.Pages.toWritableMap(): WritableMap =
    Arguments.createMap().apply {
        putInt("read", read)
        putInt("total", total)
    }

private fun SerialFields.Metadata.toWritableMap(): WritableMap =
    Arguments.createMap().apply {
...
private fun ExternalMetadataActiveInfo.toWritableMap(): WritableMap =
    Arguments.createMap().apply {
        putString("groupId", groupId)
        putString("groupName", groupName)
        putString("providerId", providerId)
...
private fun ExternalMetadataDigest.toWritableMap(): WritableMap =
    when (this) {
        is ExternalMetadataDigest.Failure ->
            Arguments.createMap().apply {
                putBoolean("isSuccess", false)
                putMap("error", error.toWritableMap())
            }
        is ExternalMetadataDigest.Success ->
            Arguments.createMap().apply {
                putBoolean("isSuccess", true)
...
private fun SerialFields.ResumePoint.toWritableMap(): WritableMap =
    Arguments.createMap().apply {
...
private fun SerialFields.Chapters.toWritableMap(): WritableMap =
    Arguments.createMap().apply {
        status?.let { putString("status", it.name) }
        readCount?.let { putInt("readCount", it) }
        putInt("total", total)
...
fun SerialDigest.toWritableMap(): WritableMap =
    when (this) {
        is SerialDigest.Failure ->
            Arguments.createMap().apply {
                putBoolean("isSuccess", false)
                putMap("error", error.toWritableMap())
            }
        is SerialDigest.Success ->
            Arguments.createMap().apply {
                putBoolean("isSuccess", true)
                putString("id", id)
                putString("name", name)
...
fun SerialsDigest.toWritableMap(): WritableMap =
    when (this) {
        is SerialsDigest.Failure ->
            Arguments.createMap().apply {
                putBoolean("isSuccess", false)
                putMap("error", error.toWritableMap())
            }
        is SerialsDigest.Success ->
            Arguments.createMap().apply {
                putBoolean("isSuccess", true)
