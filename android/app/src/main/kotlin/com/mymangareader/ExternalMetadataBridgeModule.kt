...
@Singleton
class ExternalMetadataBridgeModule @Inject constructor(
    ...
    fun listProviders(promise: Promise) {
        runCatching { externalMetadataServer.providers.list() }.resolveOrReject(promise, "LIST_PROVIDERS_ERROR") { it.toProvidersWritableArray() }
    }
    ...
    fun listGroups(promise: Promise) {
        scope.launch {
            runCatching { externalMetadataServer.groups.list() }.resolveOrReject(promise, "LIST_GROUPS_ERROR") { it.toGroupsWritableArray() }
        }
    }
    ...
    fun getGroup(groupId: String, promise: Promise) {
        scope.launch {
            runCatching { externalMetadataServer.groups.get(groupId) }.resolveOrReject(promise, "GET_GROUP_ERROR") { it?.toWritableMap() }
        }
    }

    @ReactMethod
    fun addGroup(name: String, providerId: String, credentialsJson: String, healthCheckPath: String, linkedServerGroupId: String?, promise: Promise) {
        scope.launch {
            runCatching { externalMetadataServer.groups.add(NewExternalMetadataGroup(name, providerId, credentialsJson, healthCheckPath, linkedServerGroupId)) }
                .resolveOrReject(promise, "ADD_GROUP_ERROR") { it.toWritableMap() }
        }
    }

    @ReactMethod
    fun updateGroup(groupId: String, name: String?, credentialsJson: String?, healthCheckPath: String?, linkedServerGroupId: String?, promise: Promise) {
        scope.launch {
    ...
    fun getGroupUrls(groupId: String, promise: Promise) {
        scope.launch {
            runCatching { externalMetadataServer.group(groupId).getUrls() }.resolveOrReject(promise, "GET_GROUP_URLS_ERROR") { it.toUrlsWritableArray() }
        }
    }

    @ReactMethod
    fun getGroupInfo(groupId: String, promise: Promise) {
        scope.launch {
            runCatching { externalMetadataServer.group(groupId).getInfo() }.resolveOrReject(promise, "GET_GROUP_INFO_ERROR") { it.toWritableMap() }
        }
    }

    @ReactMethod
    fun addGroupUrl(groupId: String, url: String, timeoutMs: Int, priority: Int, linkedServerUrlId: String?, promise: Promise) {
        scope.launch {
            runCatching { externalMetadataServer.group(groupId).addUrl(NewExternalMetadataUrl(url, timeoutMs, priority, linkedServerUrlId)) }
    ...
    fun getGroupActive(groupId: String, promise: Promise) {
        scope.launch {
            runCatching { externalMetadataServer.group(groupId).getActive() }.resolveOrReject(promise, "GET_GROUP_ACTIVE_ERROR") { it?.toWritableMap() }
        }
    }
    ...
    fun getActiveInfo(promise: Promise) {
        scope.launch {
            runCatching { externalMetadataServer.getActiveInfo() }.resolveOrReject(promise, "GET_ACTIVE_INFO_ERROR") { it?.toWritableMap() }
        }
    }
    ...
    fun getActiveGroupInfo(promise: Promise) {
        scope.launch {
            runCatching { externalMetadataServer.getActiveGroupInfo() }.resolveOrReject(promise, "GET_ACTIVE_GROUP_INFO_ERROR") { it?.toWritableMap() }
        }
    }
    ...
    @ReactMethod
    fun matchSync(seriesId: String, seriesName: String, promise: Promise) {
        scope.launch {
    ...
    @ReactMethod
    fun matchSyncByGroup(groupId: String, seriesId: String, seriesName: String, promise: Promise) {
        scope.launch {
    ...
    @ReactMethod
    fun matchSyncByServerId(kavitaServerGroupId: String, seriesId: String, seriesName: String, promise: Promise) {
        scope.launch {
            runCatching { externalMetadataServer.match.syncByServerId(kavitaServerGroupId, ExternalMetadataSeriesRef(seriesId, seriesName)).data }
                .resolveOrReject(promise, "MATCH_SYNC_ERROR") { it?.toWritableMap() }
        }
    }

    @ReactMethod
    fun matchSyncByServerUrl(kavitaUrl: String, seriesId: String, seriesName: String, promise: Promise) {
        scope.launch {
            runCatching { externalMetadataServer.match.syncByServerUrl(server, kavitaUrl, ExternalMetadataSeriesRef(seriesId, seriesName)).data }
                .resolveOrReject(promise, "MATCH_SYNC_ERROR") { it?.toWritableMap() }
        }
    }
    ...
    @ReactMethod
    fun matchesSync(seriesIds: ReadableArray, seriesNames: ReadableArray, promise: Promise) {
        scope.launch {
    ...
    @ReactMethod
    fun matchesSyncByGroup(groupId: String, seriesIds: ReadableArray, seriesNames: ReadableArray, promise: Promise) {
        scope.launch {
            val series = seriesIds.toSeriesRefs(seriesNames)
    ...
    @ReactMethod
    fun matchesSyncByServerId(kavitaServerGroupId: String, seriesIds: ReadableArray, seriesNames: ReadableArray, promise: Promise) {
        scope.launch {
            val series = seriesIds.toSeriesRefs(seriesNames)
    ...
    @ReactMethod
    fun matchesSyncByServerUrl(kavitaUrl: String, seriesIds: ReadableArray, seriesNames: ReadableArray, promise: Promise) {
        scope.launch {
            val series = seriesIds.toSeriesRefs(seriesNames)
    ...
    private fun ReadableArray.toSeriesRefs(names: ReadableArray): List<ExternalMetadataSeriesRef> =
        (0 until size()).map { ExternalMetadataSeriesRef(id = getString(it), name = names.getString(it)) }
    ...
    private fun List<ExternalMetadataGroupInfo>.toGroupsWritableArray() = Arguments.createArray().also { arr -> forEach { arr.pushMap(it.toWritableMap()) } }

    private fun ExternalMetadataUrlInfo.toWritableMap() = Arguments.createMap().apply {
        putString("id", id)
    ...
    private fun List<ExternalMetadataUrlInfo>.toUrlsWritableArray() = Arguments.createArray().also { arr -> forEach { arr.pushMap(it.toWritableMap()) } }

    private fun ExternalMetadataGroupFullInfo.toWritableMap() = Arguments.createMap().apply {
        putString("id", id)
        putString("name", name)
        putString("providerId", providerId)
    ...
    private fun ExternalMetadataActiveInfo.toWritableMap() = Arguments.createMap().apply {
        putString("groupId", groupId)
    ...
    private fun ExternalMetadataMatch.toWritableMap() = Arguments.createMap().apply {
    ...
    private fun List<ExternalMetadataMatch?>.toMatchesWritableArray() =
        Arguments.createArray().also { arr -> forEach { match -> if (match != null) arr.pushMap(match.toWritableMap()) else arr.pushNull() } }
}
