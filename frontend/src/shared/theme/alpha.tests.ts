...
describe('alpha', () => {
  it('turns a token into the same colour at the given opacity', () => {
    expect(alpha('rgb(255, 255, 255)', 0.15)).toBe('rgba(255, 255, 255, 0.15)');
  });

  it('works on a real token', () => {
    expect(alpha(colors.surface.dim, 0.5)).toBe('rgba(0, 0, 0, 0.5)');
  });

  it('accepts a token written without spaces', () => {
    expect(alpha('rgb(1,2,3)' as never, 0.5)).toBe('rgba(1, 2, 3, 0.5)');
  });

  it('clamps an out-of-range opacity instead of emitting an invalid colour', () => {
    expect(alpha('rgb(0, 0, 0)', 2)).toBe('rgba(0, 0, 0, 1)');
    expect(alpha('rgb(0, 0, 0)', -1)).toBe('rgba(0, 0, 0, 0)');
  });

  it('keeps full opacity meaningful', () => {
    expect(alpha('rgb(10, 20, 30)', 1)).toBe('rgba(10, 20, 30, 1)');
    expect(alpha('rgb(10, 20, 30)', 0)).toBe('rgba(10, 20, 30, 0)');
  });
  ...
  it('returns anything it cannot parse untouched', () => {
    expect(alpha('#FFFFFF' as never, 0.5)).toBe('#FFFFFF');
    expect(alpha('' as never, 0.5)).toBe('');
  });
});
